...
", 
  ...
  output$mapa.customize <- renderLeaflet({
    ...
    # Number of posible lambda values 
    ...
    output$possible.customize <- renderText({
      paste("Resolution value range indexes: 1 -", max.lambda)
    })
    
    # Get min and max lambda values 
    min.val <- lambda[1]
    max.val <- lambda[max.lambda]
    
    # Get log values
    min.log <- round(log10(min.val),3) 
    max.log <- round(log10(max.val),3)
    ...
    output$resolution.plot.customize <- renderPlot({
      ...
      yticklabels <- rev(min.log:max.log)
      ...
      # Lambda value 
      selected_value <- (max.lambda:1)[as.numeric(input$lambdavalue.customize)]
      abline(h = selected_value, col = "black", lwd=3, lty=2)
